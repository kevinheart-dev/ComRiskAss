...
class Barangay extends Model
{
    ...
    public function dataCollection()
    {
        $general = $this->generalPopulation;
        ...
        return [
            ...
            'bdrrmc_trainings' => $this->bdrrmcTrainings->map(fn($item) => [
                'title' => $item->title,
                'status' => $item->status,
                'duration' => $item->duration,
                'agency' => $item->agency,
            ...
            // Population
            'general_population' => $general,

            'population_genders' => $this->populationGenders->map(fn($item) => [
                'gender' => $item->gender,
                'value' => $item->quantity,
            ])->values(),

            'population_age_groups' => $this->populationAgeGroups->map(fn($item) => [
                'ageGroup' => $item->age_group,
                'male_no_dis' => $item->male_without_disability,
                'male_dis' => $item->male_with_disability,
                'female_no_dis' => $item->female_without_disability,
            ...
            'population_exposures' => $this->populationExposures
                ->groupBy(fn($item) => $item->hazard?->hazard_name)
                ->map(function ($group, $hazardType) {
                    return [
                        'riskType' => $hazardType,
                        'purokData' => $group->map(fn($item) => [
                            'purok' => $item->purok_number,
                            'families' => $item->total_families,
                            'individualsM' => $item->individuals_male,
                            'individualsF' => $item->individuals_female,
            ...
            // Disaster Related
            'disasters' => $this->disasterOccurances->map(function ($disaster) {
                return [
                    'disaster_name' => $disaster->disaster_name,
                    'year' => $disaster->year,
                ...
                ->map(function ($hazardGroup, $hazardName) {
                    return [
                        ...
                        'categories' => $hazardGroup->groupBy('category')->map(function ($categoryGroup, $category) {
                            return [
                                'type' => $category,
                                'rows' => $categoryGroup->map(fn($item) => [
                                    'item' => $item->item_name,
                                    'total' => $item->total_in_barangay,
                                    'percent' => $item->percentage_at_risk,
                                    'location' => $item->location,
            ...
            // Facilities & Infra
            'primary_facilities' => $this->primaryFacilities->map(fn($item) => [
                'type' => $item->facility_name,
                'quantity' => $item->quantity,
            ])->values(),
            ...
            'institutions' => $this->institutions->map(fn($item) => [
                'name' => $item->name,
                'male' => $item->male_members ?: 0,
                'female' => $item->female_members ?: 0,
                'lgbtq' => $item->lgbtq_members ?: 0,
            ...
            'road_networks' => $this->roadNetworks->map(fn($item) => [
                'type' => $item->road_type,
                'length' => $item->length_km,
                'maintained_by' => $item->maintained_by,
            ])->values(),

            'public_transportations' => $this->publicTransportations->map(fn($item) => [
                'type' => $item->transpo_type,
                'quantity' => $item->quantity,
            ])->values(),

            // Household & Housing
            'house_builds' => $this->houseBuilds->map(fn($item) => [
                'houseType' => $item->house_type,
                'oneFloor' => $item->one_floor,
                'multiFloor' => $item->two_or_more_floors,
            ])->values(),

            'house_ownerships' => $this->houseOwnerships->map(fn($ownership) => [
                'type' => $ownership->ownership_type,
                'quantity' => $ownership->quantity,
            ])->values(),
            ...
            // Livelihood
            'livelihood_statistics' => $this->livelihoodStatistics->map(fn($stat) => [
                'type' => $stat->livelihood_type,
                'male_no_dis' => $stat->male_without_disability,
                'male_dis' => $stat->male_with_disability,
                'female_no_dis' => $stat->female_without_disability,
            ...
            'livelihood_evacuation' => $this->livelihoodEvacuationSites->map(fn($site) => [
                'type' => $site->livelihood_type,
                'evacuation' => $site->evacuation_site,
                'origin' => $site->place_of_origin,
                'items' => $site->capacity_description,
            ...
            // Relief
            'relief_distributions' => $this->reliefDistributions->map(fn($relief) => [
                'evacuation_center' => $relief->evacuation_center,
                'relief_good' => $relief->relief_good,
                'quantity' => $relief->quantity,
                'unit' => $relief->unit,
            ...
            'relief_distribution_processes' => $this->reliefDistributionProcesses->map(fn($proc) => [
                'step_no' => $proc->step_no,
                'process' => $proc->distribution_process,
                'origin' => $proc->origin_of_goods,
                'remarks' => $proc->remarks,
            ...
            // Equipment & Evacuation
            'equipment_inventories' => $this->equipmentInventories->map(fn($eq) => [
                'item' => $eq->item,
            ...
            'evacuation_list' => $this->evacuationCenters->map(fn($center) => [
                'name' => $center->name,
                'families' => $center->capacity_families,
                'individuals' => $center->capacity_individuals,
                'ownerGovt' => $center->owner_type === 'government',
            ...
            'evacuation_center_inventory' => $this->evacuationInventories->map(fn($item) => [
                'totalFamilies' => $item->total_families,
                'totalIndividuals' => $item->total_individuals,
                'populationAtRiskFamilies' => $item->families_at_risk,
                'populationAtRiskIndividuals' => $item->individuals_at_risk,
            ...
            'evacuation_plans' => $this->evacuationPlans->map(fn($plan) => [
                'task' => $plan->things_to_do,
                'responsible' => $plan->responsible_person,
                'remarks' => $plan->remarks,
            ])->values(),
            ...
            // Risk & Hazard
            'families_at_risk' => $this->familiesAtRisk->map(fn($item) => [
                'value' => $item->indicator,
                'count' => $item->count,
                'purok' => $item->purok_number,
            ])->values(),

            'hazard_risks' => $this->hazardRisks->map(fn($item) => [
                'hazard' => $item->hazard?->hazard_name,
                'probability' => $item->probability_no,
                'effect' => $item->effect_no,
                'management' => $item->management_no,
            ...
            // Health
            'illnesses_stats' => $this->illnessesStats->map(fn($item) => [
                'illness' => $item->illness,
                'children' => $item->children,
                'adults' => $item->adults,
            ])->values(),

            'disability_statistics' => $this->disabilityStatistics->map(fn($item) => [
                'type' => $item->disability_type,
                'age0_6M' => $item->age_0_6_male,
                'age0_6F' => $item->age_0_6_female,
                'age7m_2yM' => $item->age_7m_2y_male,
            ...
            // Prepositioned Inventories
            'prepositioned_inventories' => $this->prepositionedInventories->map(fn($inv) => [
                'item' => $inv->item_name,
                'quantity' => $inv->quantity,
                'remarks' => $inv->remarks,
            ])->values(),
